...
function main() {
  ...
  if (!fs.existsSync(args.input_file_path)) {
    // Only the gRPC-web input file is permitted to not exist.
    if (!/grpc_web_pb\.js$/.test(args.input_file_path)) {
        throw new Error(`Internal ts_proto_library error: input file does not exist: ${args.input_file_path}`);
    }
...
function replaceRecursiveFilePaths(args) {
  return (contents) => {
    //contents = contents.replace(/..\/..\/google/g, 'google');
