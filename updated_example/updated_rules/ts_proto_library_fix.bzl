...
def _build_protoc_command(target, ctx, js_output_pairs):
    # echo_command = "echo root = %s" % target[ProtoInfo].proto_source_root
    # for p in js_output_pairs:
    #   echo_command += " (%s, %s)" % (p.import_path, p.output_file)
    ...
    # The compiler being called is declared here:
    # https://github.com/grpc/grpc-web/blob/master/javascript/net/grpc/web/grpc_generator.cc
    protoc_output_dir = _protoc_output_dir(ctx, js_output_pairs)
    protoc_command += " --grpc-web_out=import_style=commonjs+dts,mode=grpcweb:%s" % (protoc_output_dir)
    # The js proto code generator lives here:
    # https://github.com/protocolbuffers/protobuf/blob/4d6712e73995e0c64eb5f208e7388f824175b3b8/src/google/protobuf/compiler/js/js_generator.cc
...
def _protoc_output_dir(ctx, js_output_pairs):
    """Returns the directory to use when running protoc.

    The returned value should correspond to the root of the import.
    The generated files must all be declared relative to this directory.
    """
    if len(js_output_pairs) == 0:
        fail('No output files. Missing srcs?')
    dirs = []
    for spec in js_output_pairs:
        num_parts =  len(spec.import_path.split("/"))
        dir = spec.output_file.dirname
...
def _create_post_process_command(target, ctx, js_outputs, js_outputs_es6):
    ...
    for [output, output_es6] in zip(js_outputs, js_outputs_es6):
        ...
        convert_command += " --output_umd_path={}".format(output.path) # ("/dev/null")
...
def _get_outputs(target, ctx):
    ...
    proto_file_infos = _direct_source_infos(target[ProtoInfo])
    for src_info in proto_file_infos:
        src = src_info.file
        ...
        for f in generated_files:
            ...
            js_output_pairs.append(struct(
                output_file = output,
                direct_source = src,
                import_path = src_info.import_path))
    ...
    return [js_outputs, js_outputs_es6, dts_outputs, js_output_pairs]
...
def ts_proto_library_aspect_(target, ctx):
    ...
    [js_outputs, js_outputs_es6, dts_outputs, js_output_pairs] = _get_outputs(target, ctx)
    protoc_outputs = dts_outputs + js_outputs + js_outputs_es6

    all_commands = [
        _build_protoc_command(target, ctx, js_output_pairs),
...
ts_proto_library_aspect = aspect(
    ...
    attrs = {
        "_change_import_style": attr.label(
            ...
            cfg = "host",
            allow_files = True,
            default = Label("@updated_example_ws//updated_rules:change_import_style"),
        ),
        "_protoc": attr.label(
